...
int zgt_tm::TxRead(long tid, long obno, int thrNum)
 ...
 //again set the txmgr semaphore first. create a thread and first check 
...
#ifdef TM_DEBUG
   printf("\ncreating TxRead thread for Tx: %d\n", tid);fflush(stdout); //now create the thread and call the method readtx(void *)
   fflush(stdout);
#endif
   pthread_t thread1;
   ...
   int status;
   status = pthread_create(&threadid[thrNum],NULL,readtx,(void*)nodeinfo); //call the method readtx(void *)
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status);
     exit(-1);
   }
...
int zgt_tm::TxWrite(long tid, long obno, int thrNum)
{
  //call the write function (writetx); same as above
...
#ifdef TM_DEBUG
   printf("\ncreating TxWrite thread for Tx: %d\n", tid);fflush(stdout);//now create the thread and call the method writetx(void *)
   fflush(stdout);
#endif
   pthread_t thread1;
   ...
   nodeinfo->count = --SEQNUM[tid];
   int status;
   status = pthread_create(&threadid[thrNum],NULL,writetx,(void*)nodeinfo);//call the method writetx(void *)
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status);
     exit(-1);
   }
...
int zgt_tm::CommitTx(long tid, int thrNum) {
...
#ifdef TM_DEBUG
   printf("\ncreating TxCommit thread for Tx: %d\n", tid);fflush(stdout);//now create the thread and call the method committx(void *)
   fflush(stdout);
#endif
   pthread_t thread1;
   ...
   nodeinfo->count = --SEQNUM[tid];
   int status;
   status = pthread_create(&threadid[thrNum],NULL,committx,(void*)nodeinfo);//call the method committx(void *)
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status);
     exit(-1);
   }
...
{       
#ifdef TM_DEBUG
   printf("\ncreating TxAbort thread for Tx: %d\n", tid);fflush(stdout);//now create the thread and call the method aborttx(void *)
   fflush(stdout);
#endif
   pthread_t thread1;
   ...
   nodeinfo->count = --SEQNUM[tid];
   int status;
   status = pthread_create(&threadid[thrNum],NULL,aborttx,(void*)nodeinfo);//call the method aborttx(void *)
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status);
     exit(-1);
   }
